...
-- 修复 admin 用户的超级管理员权限
-- ============================================
-- 说明：确保 admin 用户关联超级管理员角色，并拥有所有系统权限

USE coal_erp;

-- 1. 查看当前 admin 用户信息
SELECT u.user_id, u.username, u.status, r.role_id, r.role_name, r.role_key
FROM sys_user u
...
LEFT JOIN sys_role_menu rm ON r.role_id = rm.role_id
WHERE u.username = 'admin'
GROUP BY u.user_id, u.username, u.status, r.role_id, r.role_name, r.role_key, r.status;
