...
- **下载链接**：
  ...
  - OpenJDK 8: https://adoptium.net/zh-CN/temurin/releases/?version=8
...
- `schema.sql` - 创建基础表结构
- 各模块的schema文件（如 `purchase_schema.sql`, `inventory_schema.sql` 等）
- 各模块的权限菜单文件（如 `purchase_permission_menu.sql` 等）
- `setup_module_managers.sql` - 创建模块负责人角色
- `create_module_manager_users.sql` - 创建模块负责人用户
- `diagnose_and_fix_system_permissions.sql` - 配置系统管理权限
...
- **前端地址**：`http://localhost:5173`（开发环境）或 `http://localhost:80`（生产环境）
- **后端API**：`http://localhost:8080`
- **默认管理员账户**：
  - 用户名：`admin`，密码：`admin123`
  - 或用户名：`admin2`，密码：`admin123`
...
**手动安装（推荐）**：
- **下载链接**：https://adoptium.net/zh-CN/temurin/releases/
- 选择对应Linux版本的JDK 8或11
...
**启动项文件夹（推荐使用）：**
- `启动项/一键启动后端.bat`：启动后端服务
- `启动项/一键启动前端.bat`：启动前端服务
- `启动项/一键编译.bat`：编译项目（Maven打包）
